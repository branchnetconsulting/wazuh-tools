...
##
## Adapted by Kevin Branch 11/8/2021 to be usable in Wazuh training lab environments
##
...
checkInstalled() {
    ...
    if [ -z "${elasticinstalled}" ]; then
        ...
        capem="${capem//$rcapem}"
	capem="/etc/elasticsearch/root-ca.pem"
...
readAdmincerts() {
    ...
    if [[ -f /etc/elasticsearch/admin.pem ]]; then
        adminpem="/etc/elasticsearch/admin.pem"
    else
        echo "Error. No admin certificate indicated. Please run the script with the option -c <path-to-certificate>."
        exit 1;
    fi

    if [[ -f /etc/elasticsearch/admin-key.pem ]]; then
        adminkey="/etc/elasticsearch/admin-key.pem"
    elif [[ -f /etc/elasticsearch/admin.key ]]; then
        adminkey="/etc/elasticsearch/admin.key"
    else
...
createBackUp() {
    ...
    # echo "./securityadmin.sh -backup /usr/share/elasticsearch/backup -nhnv -cacert ${capem} -cert ${adminpem} -key ${adminkey} -icl -h ${IP} ${VERBOSE}"
